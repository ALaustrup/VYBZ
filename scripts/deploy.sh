...
if [ -z "${NO_SUPABASE:-}" ]; then
  ...
  NO_JWT="passkey bunny-upload bunny-sign bunny-live watermark watermark-detect stripe-webhook stripe-connect-onboard stripe-tip stripe-credit-topup oauth-start oauth-callback ice-servers weekly-digest livekit-token vc-room-renewals"
