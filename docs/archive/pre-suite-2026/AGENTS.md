...
## Pick up here ΓÇö 2026-07-28

### Product lock (do not reopen)
- **VYBZ** = tip + live + catalog home for indie artists. Tagline: **Find Yours.**
- **90-day loop:** upload ΓåÆ `/u/:id` + VDock play ΓåÆ tip (`~username` / Vc) ΓåÆ live.
...
| Supabase | `xixmneooyufbeftdfpcm` (us-west-1) ΓÇö **never** point at MYVYB / other projects |
| Vercel | `astramatrix/vybz` ΓåÉ GitHub `main` |
...
1. **Prod smoke (required before alpha blast):** on https://vybz.cloud  
   Enter ΓåÆ upload track ΓåÆ play on VDock ΓåÆ tip with Vc ΓåÆ go live briefly.  
   Confirm backdrop/loops load from the `site-visuals` CDN above.
2. **Small alpha cohort** (~5ΓÇô15 artists) ΓÇö same loop; collect friction notes.
...
5. Optional: Resend tracking CNAME `mail` ΓåÆ `links1.resend-dns.com`; Google Search Console sitemap submit.
...
1. [`VYBZ_MASTERPLAN.md`](./VYBZ_MASTERPLAN.md) ΓÇö Music Hub + **GTM wedge** (wins on product)
2. This file (`AGENTS.md`) ΓÇö agent ops + pickup state
...
- **Root only** ΓÇö there is no `apps/` directory (README mention is stale).
- `npm run dev` ΓåÆ http://localhost:5173 (`server.host: true`)
- `npm run lint` ΓåÆ `tsc --noEmit`
- `npm run build` ΓåÆ `tsc --noEmit && vite build` ΓåÆ `dist/`
- `npm run visuals:encode` ΓåÆ encode masters from `vizualz/` ΓåÆ `public/`
- `npm run visuals:upload` ΓåÆ needs `SUPABASE_SERVICE_ROLE_KEY` ΓåÆ bucket `site-visuals`

### Env (client)
- Required multi-user: `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` (see `.env.example`).
- Missing Supabase env ΓåÆ app hard-stops (ΓÇ£backend not configuredΓÇ¥) ΓÇö **not** a mock offline app.
...
| Waitlist client | `src/lib/waitlist.ts` ΓåÆ EF `waitlist-join` |
...
| Bucket | Role |
|--------|------|
| **`site-visuals`** | Public CDN ΓÇö backdrop + VDock loops; anon **read**; write = service_role only |
| `media-public` | Avatars / user public media (folder = uid) |
| `audio-assets` | Private audio masters |
| `project-files` | Private project files |

CDN pattern:  
`https://xixmneooyufbeftdfpcm.supabase.co/storage/v1/object/public/site-visuals/{backdrop\|vdock/visuals}/ΓÇª`
...
- **Git LFS:** rejected on purpose ΓÇö free tier + clone bandwidth; use Storage.
- **Email:** all product mail from `@vybz.cloud` ΓÇö not `astramatrix.xyz`.
- **Living Home / dating chrome:** may still exist as stubs or demoted routes ΓÇö do not revive as the front door.
...
- **Origin-only Git:** `ALaustrup/VYBZ` ΓÇö no legacy remotes.
...
- **UX language:** frosted glass chips (`.glass-chip`), soft luminous blue atmosphere, stage-as-hero ΓÇö see reference direction in chat / `NowPlayingStage`.
