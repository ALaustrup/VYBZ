...
**identity-first** ΓÇö every account is a real creator ΓÇö but it collects as little as
...
- **Sensitive/privileged paths go through `SECURITY DEFINER` RPCs** that re-check
  `auth.uid()` and emit only aggregates + labels ΓÇö never raw private facets.
...
| public profile fields | `public_profiles` / `public_profile()` | ΓÇö |
...
not be pretend-shipped (TURN, Bunny live ingest, 8K TUS, V┬ó cash-out).
