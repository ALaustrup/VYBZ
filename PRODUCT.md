...
- **Default chrome** is VYBZ, Search, +, Chat, Alerts, and Me. **Desktop** adds one frosted PrimaryRail (Home, Messages, Live, Library — truthful routes only). **Mobile and tablet** collapse the rail into an accessible drawer and show a thumb dock strip (Home, Live, +, Chat) — the same destinations, not a second set. No permanently expanded rail on narrow screens. Library, Network, Live, and Workspace also stay reachable by URL and from owner controls. Frozen `MobileNav` and `SuiteAppRail` stay unmounted.
