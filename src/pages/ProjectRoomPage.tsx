...
export function ProjectRoomPage() {
  ...
  const needsAgree = !!(me && !me.agreed && !released);
    ...
    <div className="flex h-full flex-col">
      <div className="flex gap-5 border-b border-[var(--hairline)] px-1 pt-2">
        {TABS.map((t) => (
          <button
            ...
            onClick={() => setTab(t.id)}
            className={cx(
              "relative flex items-center gap-1.5 pb-2.5 text-[13px] font-medium transition",
              tab === t.id ? "text-white" : "text-white/40 hover:text-white/70",
            )}
          >
            <t.icon className="h-3.5 w-3.5" />
            {t.label}
            {t.id === "credits" && needsAgree && (
      ...
      {tab === "chat" ? (
        <div className="flex min-h-0 flex-1 flex-col px-1 pb-2 pt-2">
          {released && (
            <div className="mb-2 flex items-center gap-2 rounded-2xl border border-feel/25 bg-feel/[0.08] px-3.5 py-2 text-sm text-feel">
              <CheckCircle2 className="h-4 w-4 shrink-0" /> Released — credits are verified.
            </div>
          )}
          <ProjectChat projectId={id} fill />
        </div>
      ) : (
      ...
      {adding && (
        <AddCollaborator
          projectId={id}
          existing={detail.collaborators.map((c) => c.userId)}
...
function FilesTab({
  ...
  return (
    <div>
      {(detail.bpm || detail.musicalKey || detail.genres.length > 0) && (
        <div className="mb-4 flex flex-wrap gap-1.5 text-[11px]">
          ...
          {detail.genres.map((g) => (
            <span key={g} className="rounded-full bg-veil-500/20 px-2 py-0.5 text-veil-100">{g}</span>
          ))}
        </div>
      )}
      {detail.description && (
        <p className="mb-4 text-sm leading-relaxed text-white/65">{detail.description}</p>
      )}
      <SectionTitle icon={<GitBranch className="h-3.5 w-3.5" />} label="Versions" />
      {detail.versions.length === 0 ? (
        <p className="rounded-2xl border border-white/8 bg-white/[0.02] px-3.5 py-3 text-xs text-white/45">
          No versions yet. Upload the first bundle or bounce.
        </p>
      ) : (
        <div className="space-y-2">
          {detail.versions.map((v) => (
            <div key={v.id} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.03] p-3">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-veil-500/15 font-display text-sm font-bold text-veil-100">
                v{v.version}
              </span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm text-white/85">
                  {v.note || (v.format ? `${v.format.toUpperCase()} bundle` : "Version")}
                </p>
                <p className="text-[11px] text-white/40">
                  {v.uploader ?? "member"} · {new Date(v.createdAt).toLocaleDateString()}
                </p>
              </div>
            </div>
          ))}
      ...
      {canUpload && (
        <UploadVersion projectId={projectId} busy={uploading} setBusy={setUploading} onDone={onDone} />
      )}
    </div>
  );
}
...
function CreditsTab({
  ...
  detail: ProjectDetail;
  projectId: string;
  me: ProjectDetail["collaborators"][number] | undefined;
  total: number;
  allAgreed: boolean;
  released: boolean;
  onAdd: () => void;
  ...
  return (
    <div>
      ...
      <SplitSheet detail={detail} isOwner={detail.isOwner} released={released} onChange={onChange} />

      {me && !me.agreed && !released && (
        <button
          onClick={async () => {
            await api.agreeSplit(projectId);
            showToast("You agreed to your split.");
            await onChange();
          }}
          className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-feel/15 py-2.5 text-sm font-semibold text-feel ring-1 ring-feel/25 active:scale-[0.99]"
        >
      ...
      {detail.isOwner && !released && (
        <button
          onClick={onAdd}
          className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] py-2.5 text-sm font-semibold text-white/80 active:scale-[0.99]"
        >
      ...
      {detail.isOwner && !released && (
        <button
          ...
          onClick={async () => {
            await api.releaseProject(projectId);
            celebrate("Released!");
            showToast("Released — credits are now verified.");
            await onChange();
          }}
...
function SplitSheet({
  ...
  async function save() {
    ...
    try {
      ...
      setEdits({});
      showToast("Splits updated — collaborators re-agree.");
      await onChange();
    } catch {
      showToast("Couldn't update splits.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="space-y-1.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-white/90">
              ...
              {c.userId === detail.ownerId && (
                <span className="ml-1.5 text-[10px] font-normal text-veil-200">owner</span>
              )}
            </p>
            {c.role && <p className="text-[11px] text-white/45">{c.role}</p>}
          </div>
          {c.agreed ? (
            <span className="flex items-center gap-1 text-[11px] font-semibold text-feel">
          ...
          {isOwner && !released ? (
            <div className="flex items-center gap-0.5">
              <input
                ...
                min={0}
                max={100}
                value={value(c.userId, c.split)}
                onChange={(e) =>
                  setEdits((x) => ({
                    ...x,
                    [c.userId]: Math.max(0, Math.min(100, Number(e.target.value) || 0)),
                  }))
                }
      ...
      {isOwner && !released && dirty && (
        <button
          onClick={save}
          disabled={saving}
          className="mt-1 w-full rounded-xl bg-veil-500/20 py-2 text-xs font-semibold text-veil-100 active:scale-[0.99]"
        >
  ...
  return (
    <div className="mt-2 flex items-center gap-2">
      ...
      <button
        ...
        disabled={busy}
        className="flex shrink-0 items-center gap-1.5 rounded-xl bg-veil-500 px-3.5 py-2 text-sm font-semibold text-white shadow-glow active:scale-95 disabled:opacity-50"
      >
...
function AddCollaborator({
  ...
  async function add() {
    ...
    setBusy(true);
    try {
      await api.addCollaborator(projectId, picked, role);
      showToast("Collaborator added.");
      await onAdded();
    } catch {
      setBusy(false);
      showToast("Couldn't add collaborator.");
    }
  }
        ...
        {loading ? (
          ...
          <>
              ...
              {matches.map((m) => (
                <button
                  ...
                  className={cx(
                    ...
                    picked === m.userId ? "border-veil-400/60 bg-veil-500/15" : "border-white/8 bg-white/[0.03]",
                  )}
                >
                  <span className="min-w-0 flex-1 truncate text-sm font-semibold text-white/90">
                    {m.username ?? "creator"}
                  </span>
                  {m.offersYouSeek.length > 0 && (
                    <span className="truncate text-[11px] text-aqua-200">{m.offersYouSeek.slice(0, 2).join(", ")}</span>
                  )}
            ...
            <select
              ...
              className="mt-1 w-full rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm text-white"
            >
              {ROLES.map((r) => (
                <option key={r.id} value={r.id} className="bg-ink-900">{r.label}</option>
              ))}
            </select>
            <button onClick={add} disabled={!picked || busy} className="btn btn-primary mt-3 w-full py-3 disabled:opacity-40">
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Add to project"}
            </button>
          </>
        )}
      </div>
