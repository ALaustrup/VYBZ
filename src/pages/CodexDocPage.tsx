...
import { Download, Printer, Loader2, FileWarning, Info } from "lucide-react";
...
export function CodexDocPage() {
  ...
  useRegisterAppBar({
    title: doc?.title ?? "Codex",
    subtitle: doc ? `${doc.jurisdiction} · v${doc.version}` : undefined,
    actions: doc ? (
      <>
        <a href={doc.path} download={`${doc.slug}.md`}
          className="flex h-9 items-center gap-1.5 rounded-full bg-white/8 px-3 text-xs font-semibold text-white/85 active:scale-95">
          <Download className="h-3.5 w-3.5" /> .md
        </a>
        <button type="button" onClick={print}
          className="flex h-9 items-center gap-1.5 rounded-full bg-veil-500/20 px-3 text-xs font-semibold text-veil-100 active:scale-95">
          <Printer className="h-3.5 w-3.5" /> Print
        </button>
      </>
    ) : null,
  }, [doc?.title, doc?.jurisdiction, doc?.version, doc?.path, doc?.slug, html]);
  ...
  if (error || !doc) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center text-white/60">
        ...
        <button type="button" onClick={() => navigate("/codex")} className="btn btn-ghost">Back to Codex</button>
      </div>
    );
  }
