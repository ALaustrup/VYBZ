...
export function LiveSessionPanel({ session, peerName }: { session: LiveSession; peerName: string }) {
  ...
  if (state === "incoming") {
    return (
      <div className="mx-4 mb-2 flex items-center gap-2 rounded-2xl border border-veil-400/30 bg-veil-500/[0.12] px-3 py-2.5 text-sm">
        <Radio className="h-4 w-4 shrink-0 animate-pulse text-veil-200" />
        <span className="min-w-0 flex-1 truncate text-white/90"><span className="font-semibold">{peerName}</span> wants to start a live {source === "desktop" ? "desktop-audio" : "mic"} session</span>
        ...
        <button onClick={session.declineCall} aria-label="Decline" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-white active:scale-95"><X className="h-3.5 w-3.5" /></button>
      </div>
    );
  }
  ...
  return (
    <div className="mx-4 mb-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="flex-1 truncate text-sm font-semibold text-white">
          ...
          <span className="ml-1.5 text-[11px] font-normal text-white/45">{source === "desktop" ? "desktop audio" : "microphone"}</span>
        </span>
