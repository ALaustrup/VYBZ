...
export function RepoHistoryPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <p className="eyebrow mb-2">Files{selected ? " at commit" : ""}</p>
        {!tree?.entries?.length ? (
          ...
          <ul className="max-h-72 space-y-1 overflow-y-auto font-mono text-[11px] text-white/55">
            {tree.entries.slice(0, 200).map((e) => (
              <li key={e.hash + e.path} className="flex justify-between gap-2 truncate">
                <span className="truncate text-white/75">{e.path}</span>
