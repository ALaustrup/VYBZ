/**
 * Incremental SHA-256. Web Crypto is one-shot; a live mix cannot keep every
 * PCM byte in memory for a single digest() at the end.
 */
  ...
  return {
    update(data: Uint8Array) {
      ...
      if (used > 0) {
        ...
        block.set(data.subarray(0, need), used);
        compress(h, block, 0);
        used = 0;
        i = need;
      }
      ...
      if (i < data.byteLength) {
        ...
        used = data.byteLength - i;
      }
      total += data.byteLength;
    },
    hex() {
      const out = new Uint32Array(h);
      const tail = new Uint8Array(64 + 64);
      tail.set(block.subarray(0, used));
      tail[used] = 0x80;
      ...
      compress(out, tail, 0);
      return Array.from(out, (w) => w.toString(16).padStart(8, "0")).join("");
    },
  };
}
