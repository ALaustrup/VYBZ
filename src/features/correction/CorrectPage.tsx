/**
 * OR-034 Correct desk — reversible ops with bypass, before/after, loudness-matched A/B.
 * Ops: DC, peak, balance, silence, hum, width, EQ, click, loudness. Local-only.
 * A/B workbench (vs Analyzer Fix batch). Law 5: VDock dry path / simulation disclosure.
 */
...
import {
  CORRECT_OP_SUBTITLE,
  resolveCorrectOpFromQuery,
  type CorrectOp,
} from "@/features/correction/correctOps";

const OP_SUBTITLE = CORRECT_OP_SUBTITLE;
...
export function CorrectPage() {
  ...
  const [op, setOp] = useState<CorrectOp>(() => resolveCorrectOpFromQuery(searchParams.get("op")));
  ...
  const [audioSource, setAudioSource] = useState<"workspace" | "tool-drop" | null>(null);
...
/** @deprecated Prefer CorrectPage — kept for import stability during OR-034. */
export const DcOffsetCorrectPage = CorrectPage;
