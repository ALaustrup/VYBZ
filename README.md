...
**Live:** https://myvyb.astramatrix.xyz
...
set the **Site URL / redirect allow-list** to `myvyb.astramatrix.xyz` so the link
...
sender domain **must be verified in Resend first** (`astramatrix.xyz` → Resend →
...
SMTP_SENDER_EMAIL="noreply@astramatrix.xyz" SMTP_SENDER_NAME="MYVYB" \
APP_URL="https://myvyb.astramatrix.xyz" \
...
caching. Production domain: **myvyb.astramatrix.xyz**.

```bash
cd apps/veiled
vercel --prod                       # production deploy
vercel alias set <deployment-url> myvyb.astramatrix.xyz
```
